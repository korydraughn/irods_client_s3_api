...
uint64_t irods::s3::get_put_object_buffer_size_in_bytes()
{
	...
	if (!put_object_buffer_size_in_bytes.has_value()) {
		put_object_buffer_size_in_bytes =
			config.value(nlohmann::json::json_pointer{"/irods_client/put_object_buffer_size_in_bytes"}, 8192);
	}
...
uint64_t irods::s3::get_get_object_buffer_size_in_bytes()
{
	...
	if (!get_object_buffer_size_in_bytes.has_value()) {
		get_object_buffer_size_in_bytes =
			config.value(nlohmann::json::json_pointer{"/irods_client/get_object_buffer_size_in_bytes"}, 8192);
	}
...
std::string irods::s3::get_s3_region()
{
	...
	if (!s3_region.has_value()) {
		s3_region = config.value(nlohmann::json::json_pointer{"/s3_server/region"}, "us-east-1");
	}
